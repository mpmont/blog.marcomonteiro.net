...
$(window).scroll(function() {
    if (width > 480) {
        ...
        // Show Twitter widget
        var tags = $('#tags:in-viewport');

        if(tags.hasClass('tags')){
            var tweet_it = $('#tweet_it');
            if (!tweet_it.hasClass('done'))
            {
                tweet_it.fadeIn().addClass('done');
            }
        }
        $('#scrollbubble')
            .css('top', distance)
...
// Close the tweet it widget
$('.close_it').click(function() {
    $("#tweet_it").animate({ height: 'toggle', opacity: 'toggle' }, 'fast');
});
